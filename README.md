...
| `ec2-instance-count` | Optional. Used only with the `start` mode. | Number of EC2 instances to create. Default: `1`. When more than one instance is requested, the action emits `ec2-instance-id` as a JSON array. Incompatible with `use-jit: true`. |
...
| `key-pair-name` | Optional. Used only with the `start` mode. | Key pair name to attach to created EC2 instances. Useful when you need SSH access to runners for debugging or inspection. |
...
| `keep-runner-on-stop` | Optional. Used only with the `stop` mode. | When `true`, the action skips EC2 termination but still removes the runner registration from GitHub. Useful for debugging failed jobs. Default: `false`. |
...
| `ec2-instance-id`                                                                                                                                                            | EC2 Instance Id of the created runner. <br><br> When multiple runners are launched, this output contains a JSON array of instance ids. The id or ids are used to terminate the EC2 instances when the runners are not needed anymore. |
